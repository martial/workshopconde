...
void draw() {
  ...
  background(255);
    image(img, 0, 0);
  text("#HALLOWEEN", width/2, height/8);
  textSize(24);
  fill(#FFA500);
    if (millis() - time >= wait) {
  ...
  float longueurLigneTwitter = (numOfTweets*longueurLigneMax)/maxTweets;
  float longueurLigneTumblr = (totalFavorites*longueurLigneMax)/maxFavorites;
  float longueurLigneInstagram = (totalRetweets*longueurLigneMax)/maxRetweets;


  println(longueurLigneTwitter);
