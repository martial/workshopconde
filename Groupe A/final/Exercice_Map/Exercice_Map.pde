...
TembooSession session = new TembooSession("jade", "myFirstApp", "2c47c94363314ba5af6455e6b30bae15");
...
void setup() {
  ...
  maTypo = loadFont("Minecraft-48.vlw");
...
void draw() {
  ...
  fill(255,255,255,29);
  textSize(26);
  text("#Bretagne",350,664);
  
  fill(255,255,255,29);
  textSize(10);
  text("#Narbonne",650,244);
 ...
 float largeurImage = 300;
 float hauteurImage = 300;

  
  if(image.width > 0) {
  image(image, 100,100);
  
 image(image, 40,40, largeurImage, hauteurImage);
  ...
  for ( int i=0; i<markers.size (); i++ ) {
    ...
    if (colors.size() > i ) {
      ...
      fill(c);
      rect(position.x, position.y, 14, 14);
    }
  }
}
...
void runSearchPhotosChoreo(float latitude, float longitude) {
  ...
  // Set credential
  searchPhotosChoreo.setCredential("hhhh");
