class ImageHelper  {
  ...
  ArrayList getPalette (PImage img) {

    ArrayList hist=createHistogramFromImage(img.pixels, img.pixels.length/2, 0.35, true);
    ArrayList avgColors = new ArrayList();
    //avgTColors = new ArrayList();
    float x=0;
    float w=(float)width/hist.size();
    for (Iterator<HistEntry> i=hist.iterator(); i.hasNext() && x<width;) {
      HistEntry e=i.next();
      //avgColors.add((int)e.col.toARGB());
      avgColors.add(e.col);
    }
   ...
   ArrayList getMoviePalette (GSMovie mov) {
     
    // mov.loadPixels();

    ArrayList hist=createHistogramFromImage(mov.pixels, mov.pixels.length/2, 0.35, true);
    ArrayList avgColors = new ArrayList();
    //avgTColors = new ArrayList();
    float x=0;
    float w=(float)width/hist.size();
